...
- Types: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`,
  `build`, `ci`, `revert`.
- Scope: the provider under `rero_invenio_thumbnails/contrib/` that the change
  touches (`bnf`, `dnb`, `google_books`, `open_library`, `amazon`,
  `internet_archive`). Outside `contrib/`, use the area, as the history already
  does — `http`, `deps`, `dev`. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(bnf)!: ...`.
