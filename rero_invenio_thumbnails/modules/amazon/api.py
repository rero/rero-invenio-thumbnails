...
class AmazonProvider:
    ...
    def get_thumbnail_url(self, isbn):
        ...
        """
        try:
            # We need ISBN 10 in Amazon
            isbn = to_isbn10(isbn)
            url = f"https://www.amazon.com/gp/product/{isbn}"
            image_url = f"https://images-na.ssl-images-amazon.com/images/P/{isbn}.{self.country}._{self.size}_.jpg"
            headers = {
                "User-Agent": "Mozilla/5.0",
                "Accept-Language": "en-US,en;q=0.9",
            }
            response = requests_retry_session().get(url, headers=headers, timeout=5)
            status_code = response.status_code
            if status_code == requests.codes.ok:
                return image_url
            # log debug for non-200 responses
            current_app.logger.debug(f"ISBN not found Amazon {isbn}: {status_code}")
            return None

        except ValueError as e:
            current_app.logger.warning(f"Invalid ISBN format for Amazon provider: {isbn}: {e!s}")
        except requests.RequestException as e:
            current_app.logger.error(f"Request error retrieving thumbnail for ISBN {isbn} from Amazon: {e!s}")
        except Exception as e:
            current_app.logger.error(f"Unexpected error retrieving thumbnail for ISBN {isbn} from Amazon: {e!s}")
        return None
