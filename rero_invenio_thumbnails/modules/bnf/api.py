...
class BnfProvider:
    ...
    def __init__(self, app_name="NE", cover_page=1):
        """Initialize the BNF provider.

        :param app_name: Application name identifier for BNF API. Must be "NE" as per BNF requirements.
        :param cover_page: Cover page to retrieve (1=front cover, 4=back cover). Defaults to 1.

        Examples:
            >>> # Default provider (front cover)
            >>> provider = BnfProvider()
            >>> # Back cover
            >>> provider = BnfProvider(cover_page=4)
        """
    ...
    def get_thumbnail_url(self, isbn):
        ...
        Examples:
            ...
            >>> # Using clean ISBN
            >>> url = provider.get_thumbnail_url("9782070360284")
        ...
        Note:
            ...
            - Returns None if ISBN cannot be converted to ARK or cover doesn't exist.
        ...
        url = f"http://catalogue.bnf.fr/couverture?appName={self.app_name}&idArk={ark_id}&couverture={self.cover_page}"
