    ...
    def get_thumbnail_path(self, isbn):
        ...
        try:
            ...
            for ext in supported_extensions:
                thumbnail_path = os.path.join(files_dir, f"{isbn}{ext}")

                if os.path.isfile(thumbnail_path):
                    return thumbnail_path

            current_app.logger.debug(f"No thumbnail found for ISBN {isbn} in {files_dir}")
            return None

        except Exception as e:
            current_app.logger.error(f"Error retrieving thumbnail path for ISBN {isbn}: {e!s}")
            return None
    ...
    def get_thumbnail_url(self, isbn):
        ...
        try:
            ...
            if url := current_app.config.get("RERO_ILS_URL"):
                return f"{url}/api/thumbnails/{isbn}"
            return f"http://localhost/api/thumbnails/{isbn}"

        except Exception as e:
            current_app.logger.error(f"Error generating thumbnail URL for ISBN {isbn}: {e!s}")
            return None
