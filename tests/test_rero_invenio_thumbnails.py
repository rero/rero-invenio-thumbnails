...
class TestGetThumbnailUrl:
    ...
    def test_get_thumbnail_url_multiple_providers_first_success(self, app):
        ...
        with app.app_context(), patch("rero_invenio_thumbnails.api.PROVIDERS") as mock_providers:
            ...
            # Setup configuration
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["files", "amazon", "open_library"]
...
class TestBlueprintEndpoint:
    ...
    def test_endpoint_get_thumbnail_not_found(self, app, client):
        ...
        with app.app_context(), tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "nonexistent-isbn" in data["isbn"]
    ...
    def test_endpoint_cached_parameter_false(self, app, client):
        """Test that cached=false parameter disables caching."""
        with (
            app.app_context(),
            patch("rero_invenio_thumbnails.api.PROVIDERS") as mock_providers,
            patch("rero_invenio_thumbnails.api.requests_retry_session") as mock_session,
        ):
            ...
            # Mock the provider and requests
            ...
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["open_library"]
            ...
            initial_call_count = mock_session.return_value.get.call_count
            ...
            assert mock_session.return_value.get.call_count == initial_call_count + 1
...
class TestGetThumbnailFunction:
    ...
    def test_get_thumbnail_with_remote_url_and_resizing(self, app):
    ...
    def test_get_thumbnail_with_remote_url_no_resizing(self, app):
    ...
    def test_get_thumbnail_with_fetch_exception(self, app):
    ...
    def test_get_thumbnail_local_file_exception_with_url_fallback(self, app):
...
class TestIntegration:
    ...
    def test_open_library_non_image_content_type(self, app):
        """Test OpenLibraryProvider rejects non-image content types."""
        with app.app_context():
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["open_library"]

            with patch("rero_invenio_thumbnails.modules.open_library.api.requests_retry_session") as mock_session:
                mock_response = MagicMock()
                mock_response.status_code = 200
    ...
    def test_open_library_request_exception(self, app):
    ...
    def test_request_retry_session_with_header(self):
        """Test requests_retry_session with custom headers."""
        from rero_invenio_thumbnails.modules.utils import requests_retry_session

        session = requests_retry_session(retries=3, backoff_factor=0.5, headers={"User-Agent": "TestAgent/1.0"})
        assert session is not None
        assert "http://" in list(session.adapters)
        assert "https://" in list(session.adapters)
    ...
    def test_amazon_provider_json_parse_error(self, app):
        """Test AmazonProvider handles parsing errors gracefully."""
        with app.app_context():
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["amazon"]

            with patch("rero_invenio_thumbnails.modules.amazon.api.requests_retry_session") as mock_session:
                mock_session.return_value.get.side_effect = ValueError("Invalid HTML")

                url = get_thumbnail_url("9780134685991")
                assert url is None

    def test_google_api_provider_http_error(self, app):
        """Test GoogleApiProvider handles HTTP errors."""
        with app.app_context():
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["google_api"]

            with patch("rero_invenio_thumbnails.modules.google_api.api.requests_retry_session") as mock_session:
                mock_session.return_value.get.side_effect = Exception("HTTP Error")

                url = get_thumbnail_url("9780134685991")
                assert url is None

    def test_google_books_provider_empty_response(self, app):
        """Test GoogleBooksProvider handles empty JSON response."""
        with app.app_context():
            app.config["RERO_INVENIO_THUMBNAILS_PROVIDERS"] = ["google_books"]

            with patch("rero_invenio_thumbnails.modules.google_books.api.requests_retry_session") as mock_session:
                mock_response = MagicMock()
                mock_response.status_code = 200
                mock_response.text = ""
                mock_session.return_value.get.return_value = mock_response
    ...
    def test_amazon_provider_timeout(self, app):
    ...
    def test_google_books_provider_malformed_jsonp(self, app):
        ...
        with app.app_context():
            ...
            with patch("rero_invenio_thumbnails.modules.google_books.api.requests_retry_session") as mock_session:
                mock_response = MagicMock()
                mock_response.status_code = 200
                mock_response.text = "notjsoncallback({invalid json})"
                mock_session.return_value.get.return_value = mock_response

                url = get_thumbnail_url("9780134685991")
                assert url is None

    def test_open_library_provider_404_status(self, app):
