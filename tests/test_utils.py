...
class TestFilesProviderCoverage:
    ...
    def test_get_thumbnail_url_with_rero_ils_url(self, app):
        ...
        with app.app_context(), tempfile.TemporaryDirectory() as temp_dir:
            ...
            url = provider.get_thumbnail_url(test_isbn)
            assert url == "https://example.com/api/thumbnails/9780134685991"
...
class TestApiCoverage:
    ...
    def test_filesystem_cache_corrupted_data(self, app):
        """Test FilesystemCache.get with corrupted cache data."""
        from rero_invenio_thumbnails.api import FilesystemCache

        with app.app_context(), tempfile.TemporaryDirectory() as temp_dir:
            app.config["RERO_INVENIO_THUMBNAILS_CACHE_DIR"] = temp_dir
            cache = FilesystemCache()
            cache_key = "test_corrupted"

            # Create corrupted cache file (less than 8 bytes for timestamp)
            cache_path = cache.cache_dir / f"{cache_key}.cache"
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            with open(cache_path, "wb") as file:
                file.write(b"bad")  # Less than 8 bytes

            result = cache.get(cache_key)
            assert result is None

    def test_get_thumbnail_invalid_provider(self, app):
        """Test get_thumbnail with invalid provider name raises KeyError."""
